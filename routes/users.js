...
router.post('/login', (req, res, next) => {
  ...
  db.getConnection().then(connection => {
    outCon = connection
    return db.execute(connection, 'select user_id, user_pass, user_name, settings from t_user where user_id = ? and del_flag = 0', [userId])
    ...
    const {token, expireDate} = util.encodeToken({userId: userInfo.userId})
